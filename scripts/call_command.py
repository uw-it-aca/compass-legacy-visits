...
class CallCommand:
    ...
    def __init__(self, *args, **kwargs):
        ...
        metric_name_prefix = 'task{}_'.format(
    ...
    def run(self):
        ...
        while True:
            ...
            rv = -1
            try:
                rv = check_output([self.command, *self.options])
...
class Metrics:
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.job = os.path.basename(os.path.splitext(command)[0])
    ...
    def collect(self, **kwargs):
        for key, value in kwargs.items():
            try:
                self.metrics[
                    key].labels(self.job, self.release_id).set(value)
            except KeyError:
